...
export default {
  ...
  methods: {
    ...mapMutations("song", ["setCurrentPlayTime"]),
    ...mapActions("song", ["getCurrentSongUrl"]),
    //判断当前歌曲是否被收藏
    isLove() {
      ...
      if (
        ...
        JSON.parse(localStorage.getItem("currentSongInfo"))
      ) {
        axios
          .get(
            "http://43.140.252.215:8080/music/playlist/isPlaylistByUserIdAndMusicId/" +
    ...
    addtoPlayList() {
      ...
      if (JSON.parse(localStorage.getItem("currentSongInfo")).ar[1]) {
        Param.artists +=
          "、" + JSON.parse(localStorage.getItem("currentSongInfo")).ar[1].name;
        Param.artistsId +=
          "、" + JSON.parse(localStorage.getItem("currentSongInfo")).ar[1].id;
      }
      console.log(Param);
      axios
        .post("http://43.140.252.215:8080/music/details/addDetails", Param)
        .then((res) => {
          console.log(res);
          if (res) {
            this.loveColor = "red";
    ...
    delfromPlayList() {
      axios
        .post(
          "http://43.140.252.215:8080/music/details/deleteDetails/" +
            JSON.parse(localStorage.getItem("userInfo")).userId +
            "/" +
            JSON.parse(localStorage.getItem("currentSongInfo")).id
        )
    ...
    async musicPlay() {
      ...
      //获取这首歌的信息
      await this.$api.song
        .detail({ ids: this.currentSongInfo.id + "" })
        .then((res) => {
          console.log(res);
          var Param = {
            /** 歌曲id */
          ...
          axios
            .post(
              "http://43.140.252.215:8080/music/playrecord/saveNewPlayRecord",
              Param2
            )
            .then((res) => {
              console.log(res);
    ...
    playNext() {
      console.log("播放下一首");
      const index = this.waitingPlayList.findIndex(
    ...
    playPrev() {
      ...
      if (index > 0) {
        ...
        this.$playMusic(
          this.waitingPlayList[this.waitingPlayList.length - 1].id
        );
      }
    },
    // 随机播放
    shufflePlay() {
      ...
      function getRandomId() {
        let id =
          waitingPlayList[Math.floor(Math.random() * waitingPlayList.length)]
            .id;
    ...
    handleNextPrev(type = "next", isEnded = false) {
      ...
      switch (this.$store.getters.playType) {
        case "orderPlay":
          // 顺序播放
          type == "next" ? this.playNext() : this.playPrev();
          break;
        case "singleCycle":
          // 单曲循环
          if (isEnded) {
    ...
    bindMediaObject() {
      if (
        ...
        Object.keys(this.currentSongInfo).length
      ) {
        navigator.mediaSession.metadata = new MediaMetadata({
          ...
          artwork: [
            {
              src: this.currentSongInfo.al.picUrl + "?param=512y512",
              sizes: "512x512",
              type: "image/png",
            },
          ],
  ...
  computed: {
    ...mapState("song", ["currentSongInfo", "waitingPlayList", "musicUrl"]),
    endTime() {
