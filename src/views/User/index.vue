  ...
  <div class="personal-center">
    <v-row>
      <v-col cols="12" md="4">
        <v-card class="mx-auto" max-width="344">
          ...
          <v-card-actions>
            <v-btn color="green lighten-2" dark @click="modifyPasswordDialog = true">
              修改密码
            </v-btn>
    ...
    <v-dialog v-model="dialog" persistent width="300">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-btn :color="$store.getters.mainColor" :loading="loading" text @click="logout">
            确定
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="modifyPasswordDialog" persistent width="300">
      <v-card>
        ...
        <span class="sp">原密码：</span><el-input class="pass" placeholder="请输入原密码" v-model="oldpass"
          show-password></el-input>
        <br>
        <span class="sp">新密码：</span><el-input class="pass" placeholder="请输入新密码" v-model="newpass"
          show-password></el-input>
        <br>
        <span class="sp">确认密码：</span><el-input class="pass" placeholder="确认密码" v-model="repass" show-password></el-input>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn :color="'#333'" text @click="!modifyPasswordLoading && (modifyPasswordDialog = false)">
            取消
          </v-btn>
          <v-btn :color="$store.getters.mainColor" :loading="modifyPasswordLoading" text @click="modifyPassword">
            确定
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="modifyInfoDialog" persistent width="300">
      <v-card>
        ...
        <el-input style="margin-left: 10%;width: 80%;" placeholder="请输入昵称" v-model="newName" clearable>
        </el-input>

        <br>
        <el-input style="margin-left: 10%;width: 80%;" type="textarea" :rows="2" placeholder="请输入简介"
          v-model="newIntroduction">
        </el-input>

        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn :color="'#333'" text @click="!modifyInfoLoading && (modifyInfoDialog = false)">
            取消
          </v-btn>
          <v-btn :color="$store.getters.mainColor" :loading="modifyInfoLoading" text @click="modifyInfo">
            确定
          </v-btn>
        </v-card-actions>
      </v-card>
...
export default {
  ...
  methods: {
    ...
    modifyPassword() {
      ...
      axios.post('http://43.140.252.215:8080/music/info/modifyPassword/' + JSON.parse(localStorage.getItem("userInfo")).userId + '/' + this.oldpass + '/' + this.newpass).then(res => {
    ...
    modifyInfo() {
      ...
      axios.post('http://43.140.252.215:8080/music/info/modifyInfo/' + JSON.parse(localStorage.getItem("userInfo")).userId + '/' + this.newName + '/' + this.newIntroduction).then(res => {
