    ...
    <div class="playlist-header">
      ...
      <div class="text-box" :style="{width: `calc(100% - ${imgSize}px)`}">
        <v-skeleton-loader
          ...
          :loading="loading"
        >
          <div class="name text-overflow2">{{ name }}</div>
          <v-btn
            small
            class="play-all"
  ...
  methods: {
    ...
    async getData() {
      ...
      this.listError = false
       ...
       await axios.get("http://43.140.252.215:808080/music/playlist/getPlaylistById/" + JSON.parse(localStorage.getItem("userInfo")).userId).then(res => {
     ...
     async renderPlaylist(data) {
      ...
      await axios.get("http://43.140.252.215:808080/music/details/getDetailsById/"+data.id).then(res => {
